...
-   **Under strict order:** $A_1 \leq \cdots, \leq A_n$
    -   Nicholls & Jones applied directly to ages (see section 4 in @nicholls2001 )
    -   Approached Jeffreys:
...
It is important to note that only BayLum prior accepts partial order constraints. For others priors there is no need to specify the parameter `StratiConstraints` as it is supposed to be the **strict order** or no constraints at all. 
...
Then the following code snippet shows when we use the *approached Jeffrey* prior for the second bayesian model
