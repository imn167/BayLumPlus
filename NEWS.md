...
## Major New Features
- Graph-based Stratigraphic Tools: New functions to build, visualize, and simplify stratigraphic relationships 
using graph theory.

- Advanced Bayesian Age Modeling: A new function, Compute_AgeS_D(), for more flexible age estimation with 
multiple prior structures.
...
to visually compare Highest Posterior Density (HPD) intervals across different model settings.
...
The `Compute_AgeS_D()` function adds two new outputs (standard deviation and time-series standard error), 
which may affect scripts expecting the original output format.
