...
| `DR_TOTAL`       | total dose rate                                          | Gy/ka (\$\$\dot{d}\$\$ ) |
| `DR_TOTAL_X`     | standard error total dose rate (\$\$\sigma\_\dot(d)\$\$) | Gy/ka                    |
...
| `s_CAL`     | relative uncertainty beta-source calibration \_lab                      | \-    |
| `s_intDR`   | absolute uncertainty internal dose rate                                 | Gy/ka |
| `s_water`   | Relative uncertainty due to water conc (\$\$\sigma\_{\text{water}}\$\$) |       |
